...
def solution(S, P, Q):
    N = len(S)
    letters = 'ACG'  # T is the fallback
    
    # prefix[c][i] = count of letter c in S[0..i-1]
    prefix = {c: [0] * (N + 1) for c in letters}
    for i, ch in enumerate(S):
        for c in letters:
            prefix[c][i + 1] = prefix[c][i] + (ch == c)
    
    result = []
    for p, q in zip(P, Q):
        for impact, c in enumerate(letters, start=1):
            if prefix[c][q + 1] - prefix[c][p] > 0:
